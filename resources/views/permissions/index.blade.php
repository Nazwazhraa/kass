...
@section('content')
    <div class="container p-4">
        ...
        <div class="flex flex-wrap mb-4">
            <div class="mr-6">
                <a href="{{ route('permissions.create') }}" class="relative inline-block font-medium group mb-2">
                    <span
        ...
        <div class="flex justify-between">
            <form action="{{ route('permissions.index') }}" method="GET" class="py-2">
            ...
            <table class="w-full text-sm text-left border-2 border-gray-200 ">
                ...
                <tbody>
                    @foreach ($permissions as $permission)
                        <tr class="bg-white border-b dark:bg-gray-900 dark:border-gray-700" id="permissions_ids{{ $permission->id}}">
                            <td class="pl-4">
                                <input type="checkbox" name="ids" class="checkbox_ids" value="{{$permission->id}}">
                            </td>
                            <th scope="row"
        ...
        <div class="flex justify-between">
            <div class="py-3">
                <div class="text-gray-600 text-sm mt-4">
                    Showing {{ ($permissions->currentPage() - 1) * $permissions->perPage() + 1 }} to
                    {{ ($permissions->currentPage() - 1) * $permissions->perPage() + $permissions->count() }} of
                ...
                <div class="btn-group">
                    ...
                    @for ($i = 1; $i <= $permissions->lastPage(); $i++)
                        <a href="{{ $permissions->url($i) }}"
                            class="bg-gray-300 hover:bg-gray-400 text-gray-800 font-bold py-2 px-4  {{ $i == $permissions->currentPage() ? 'current' : '' }}">{{ $i }}</a>
                    @endfor
